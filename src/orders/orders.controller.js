...
function checkOrder (req, res, next) {
  ...
  dishes.forEach((dish, index) => {
    if (!dish.quantity || dish.quantity <= 0 || typeof dish.quantity != "number") return next({ status: 400, message: `Dish ${index} must have a quantity that is an integer greater than 0` });
  });
...
function update (req, res, next) {
  ...
  if(!status || status == "") return next({ status: 400, message: `Order must include a status` });
  if(status === "invalid") return next({ status: 400, message: `Order status must be valid` });
  if(status === "delivered") return next({ status: 400, message: `A delivered order cannot be changed.` });
...
function destroy (req, res, next) {
  ...
  if(foundOrder > -1) orders.splice(foundOrder, 1);
  res.sendStatus(204);
};
